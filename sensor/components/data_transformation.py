...
class DataTransformation:
    ...
    def get_data_transformer_object(cls) -> Pipeline:
        try:
            ...
            preprocessor = Pipeline(
                steps=[
                    ("imputer", simple_imputer), # Fill missing values with constant number "0"
                    ("RobustScaler", robust_scaler) # Keeps every feature in same range and handles outliers
                ]
            )
    ...
    def initiate_data_transformation(self,) -> DataTransformationArtifact:
        try:
            ...
            # For testing dataset
            input_feature_test_df = test_df.drop(columns=[TARGET_COLUMN], axis=1, inplace=False)

            target_feature_test_df = test_df[TARGET_COLUMN]
            target_feature_test_df = target_feature_test_df.replace(TargetValueMapping().to_dict())  

            # Performing fit and transform on train data
            preprocessor_object = preprocessor.fit(input_feature_train_df)
            transformed_input_train_feature = preprocessor_object.transform(input_feature_train_df)

            # Performing transform on test data
            ...
            # Concatenating final datasets after balancing
            train_arr = np.c_[input_feature_train_final, np.array(target_feature_train_final)]
            test_arr = np.c_[input_feature_test_final, np.array(target_feature_test_final)]  

            # Saving the objects and training and testing numpy arrays
